    ...
    try {
      for (const item of importPreview.accounts) {
        try {
          ...
          successCount += 1;
          if (!firstImportedAccountId) firstImportedAccountId = account.id;
        } catch (error) {
          failures.push({
            row: item.row,
            name: item.name,
      ...
      if (firstImportedAccountId) await switchAccount(firstImportedAccountId);

      setImportFailures(failures);
      if (successCount > 0) message.success(`已导入 ${successCount} 个店铺`);
      if (failures.length > 0) message.warning(`${failures.length} 个店铺导入失败`);
    } finally {
      setImporting(false);
    }
  };
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      >
        <div style={stackedLayoutStyle}>
          ...
          <pre style={importExampleStyle}>{ACCOUNT_IMPORT_EXAMPLE}</pre>
