...
interface Props {
  ...
  onOpenTaobaoOrder?: (platformOrderId: string) => void;
}
...
export const OrderAssociationModal: React.FC<Props> = ({ open, association, saving, onCancel, onSave, onOpenTaobaoOrder }) => {
  ...
  const handleOpenTaobaoOrder = () => {
    ...
    onOpenTaobaoOrder?.(platformOrderId);
  };
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" initialValues={DEFAULT_VALUE}>
        ...
        <Button type="dashed" block onClick={handleOpenTaobaoOrder} style={{ marginBottom: 16 }}>
          打开淘宝订单页
        </Button>
