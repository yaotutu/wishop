          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <Space size={4} align="center">
                <Text type="secondary" onClick={() => options.onCopyText(record.order_id, '订单号')} style={{ fontSize: 12, cursor: 'pointer' }}>
                  {record.order_id}
                </Text>
                <Button type="text" size="small" icon={<CopyOutlined />} onClick={() => options.onCopyText(record.order_id, '订单号')} style={{ width: 18, height: 18, minWidth: 18, padding: 0 }} />
              </Space>
              <div style={{ fontSize: 13, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{product?.title || '-'}</div>
              <Space size={4} style={{ marginTop: 2 }}>
                <Button type="link" size="small" icon={<PictureOutlined />} disabled={!product?.thumb_img} onClick={() => options.onCopyImage(product?.thumb_img)} style={{ padding: 0, height: 20, fontSize: 12 }}>
                  复制图片
                </Button>
                <Button type="link" size="small" icon={<CopyOutlined />} disabled={!product?.title} onClick={() => options.onCopyText(product?.title, '标题')} style={{ padding: 0, height: 20, fontSize: 12 }}>
                  复制标题
                </Button>
              </Space>
              <br />
              {product && (
                <Text type="secondary" onClick={() => options.onCopyText(specs || undefined, '规格')} style={{ fontSize: 12, cursor: specs ? 'pointer' : 'default', maxWidth: 185, display: 'block' }} ellipsis title={specs || '无规格'}>
                  规格: {specs || '无规格'} ｜ x{product.sku_cnt}
                </Text>
              )}
              {product?.sku_code && (
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, maxWidth: '100%' }}>
                  <Text type="secondary" onClick={() => options.onCopyText(product.sku_code, 'SKU')} style={{ fontSize: 12, cursor: 'pointer', maxWidth: 145 }} ellipsis title={`编码: ${product.sku_code}`}>
                    编码: {product.sku_code}
                  </Text>
                  <Button type="link" size="small" icon={<CopyOutlined />} onClick={() => options.onCopyText(product.sku_code, 'SKU')} style={{ width: 18, height: 18, minWidth: 18, padding: 0, flexShrink: 0 }} />
                </span>
              )}
              <br />
            ...
            {linked ? (
              <>
                ...
                {canPrepareTaobaoRefund && (
                  <Button type="link" size="small" danger icon={<RollbackOutlined />} loading={options.preparingTaobaoRefundOrderIds.has(record.order_id)} onClick={() => options.onPrepareTaobaoRefund(record)} style={{ padding: 0, height: 18, fontSize: 12, justifySelf: 'start' }}>
                    申请退款
                  </Button>
                )}
