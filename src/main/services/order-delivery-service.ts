...
function matchDeliveryCompany(logisticsCompany: string, companies: DeliveryCompany[]): DeliveryCompany | null {
...
export async function shipOrderFromPurchase(input: ShipOrderFromPurchaseInput, deps: OrderDeliveryDeps): Promise<ShipOrderFromPurchaseResult> {
  ...
  deps.addGlobalLog({
    ...
    metadata: { orderId: input.orderId, source: 'purchaseLookup' },
  });
  ...
  try {
    ...
    deps.addGlobalLog({
      ...
      metadata: { orderId: input.orderId, deliveryId: company.delivery_id, source: 'purchaseLookup' },
    });
